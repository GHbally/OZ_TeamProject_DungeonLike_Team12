...
public class StageMapSwitcher : MonoBehaviour
{
    ...
    public void ChangeMap(int stage)
    {
        ...
        if (SoundManager.Instance != null)
        {
            ...
            {
                case 1: //1 스테이지일 때
                    SoundManager.Instance.PlayBGM("A Great Journey - Overworld", 0.15f);
                    break;

                case 2: //2 스테이지일 때
                    SoundManager.Instance.PlayBGM("Gearing Up - Battle", 0.15f);
                    break;

                case 3: //3 스테이지(예: 보스 방)일 때
                    SoundManager.Instance.PlayBGM("The Beast's Lair - Boss Fight", 0.15f);
                    break;
